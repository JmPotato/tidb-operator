...
properties([
    parameters([
        ...
        string(name: 'RELEASE_VER', defaultValue: '', description: "the version string in released tarball, e.g. v1.2-nightly"),
                    ...
                    dir("${WORKSPACE}/go/src/github.com/pingcap/tidb-operator") {
                        ...
                        stage('Run') {
                            ...
                            string(credentialsId: "tp-codecov-token", variable: 'CODECOV_TOKEN')
                                ...
                                export SRC_BRANCH=${env.ghprbSourceBranch}
                                export GIT_COMMIT=${env.ghprbActualCommit}
                                export PR_ID=${env.ghprbPullId}
...
try {
    ...
    timeout (time: 2, unit: 'HOURS') {
                ...
                dir("${PROJECT_DIR}") {
                    ...
                    stage("Prepare for e2e") {
                        withCredentials([
                            ...
                            BUILD_NUMBER=${BUILD_NUMBER} SRC_BRANCH=${env.ghprbSourceBranch} GIT_COMMIT=${env.ghprbActualCommit} PR_ID=${env.ghprbPullId} CODECOV_TOKEN=${CODECOV_TOKEN} \
                            ./hack/e2e-patch.sh || echo "e2e patch script not found, ignore"
                            ...
                            SRC_BRANCH=${env.ghprbSourceBranch} GIT_COMMIT=${env.ghprbActualCommit} PR_ID=${env.ghprbPullId} \
        ...
        def GLOBALS = "KIND_ETCD_DATADIR=/mnt/tmpfs/etcd E2E=y SKIP_BUILD=y SKIP_IMAGE_BUILD=y DOCKER_REPO=hub.pingcap.net/tidb-operator-e2e IMAGE_TAG=${IMAGE_TAG} DELETE_NAMESPACE_ON_FAILURE=true GINKGO_NO_COLOR=y"
